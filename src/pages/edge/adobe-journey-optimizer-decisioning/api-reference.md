...
Starting from Android SDK version 3.4.0 and IOS SDK version5.4.0 the Optimize SDK provides enhanced support for batching multiple display propositions track events. The following methods are available in `OfferUtils.kt` and `Optimize.swift` public classes respectively.
