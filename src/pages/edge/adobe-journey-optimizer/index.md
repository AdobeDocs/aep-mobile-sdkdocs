...
| messaging.pushForceSync | No | Allows the push token to be synced with Adobe Journey Optimizer every time the `setPushIdentifier` API is called. More details can be found in the [push token sync improvements documentation](./push-notification/push-token-sync-improvements.md) | Boolean | Android/iOS |
