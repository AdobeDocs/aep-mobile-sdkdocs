...
Start Lifecycle data collection by calling `lifecycleStart(_:)` from within the callback of the `MobileCore.registerExtensions` method in your app's `application(_:didFinishLaunchingWithOptions:)` delegate method.
...
For a scene-based UI, call `lifecycleStart(_:)` from the `UISceneDelegate`'s `sceneWillEnterForeground(_:)` method.

For all other apps, call `lifecycleStart(_:)` from the `UIApplicationDelegate`'s `applicationWillEnterForeground(_:)` method.
...
First, register the Lifecycle extension and configure the Mobile SDK from the `App` class.
...
Next, set the `@Environment` property wrapper used to listen for lifecycle changes.
...
Finally, use the `scenePhase` property in conjunction with `.onChange(of:)`.
...
You can use these callbacks to ensure that all of your `Activities` correctly call the Lifecycle APIs, and do not need to implement the code for each of the Activity. Add code to register an instance of `ActivityLifecycleCallbacks` in your `Application` class, just before [registering your extensions](#register-lifecycle-with-mobile-core) with `MobileCore`.
